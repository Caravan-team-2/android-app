...
sealed class InputError(open val field: Field, open val text: Int) {
    ...
    data class Required(override val field: Field, override val text: Int) : InputError(field, text)
    data class InvalidDate(override val field: Field, override val text: Int) : InputError(field, text)
}